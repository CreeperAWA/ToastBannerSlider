...
这是一个用于生成软件许可证的图形界面工具。
支持输入机器码、设置有效期、通过日历选择时间等功能。
"""
...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, 
                               ...
                               QMessageBox, QGroupBox, QFormLayout, QSizePolicy, QFrame, QScrollArea,
                               QGridLayout, QToolButton, QStyle)
from PySide6.QtCore import QDate, Qt, QTimer, QPropertyAnimation, QEasingCurve, QPoint, QRect
from PySide6.QtGui import QFont, QIcon, QPalette, QColor, QPainter, QLinearGradient, QPixmap, QBrush, QCursor, QMovie
...
class LicenseGeneratorUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.days_spinbox.setRange(1, 2912530)
    ...
    def load_current_hardware_info(self):
        """加载当前机器的硬件信息"""
        try:
            hardware_key = self.license_manager.generate_hardware_key()
    ...
    def generate_license(self):
        ...
        try:
            ...
            # 创建临时许可证生成器并设置硬件标识
            temp_generator = LicenseGenerator()
            
            # 保存原始方法
            original_get_hardware_info = temp_generator.get_hardware_info
            original_generate_hardware_key = temp_generator.generate_hardware_key
            
            # 重写方法以使用输入的硬件标识
            def mock_get_hardware_info():
                return self.license_manager.get_hardware_info()
                
            def mock_generate_hardware_key():
                return hardware_key
                
            temp_generator.get_hardware_info = mock_get_hardware_info
            temp_generator.generate_hardware_key = mock_generate_hardware_key
            
            # 生成许可证
            success = temp_generator.generate_license(licensee, days)
