...
from PySide6.QtCore import (Qt, QPropertyAnimation, QEasingCurve, QPoint, 
                            QTimer, Signal)
from PySide6.QtGui import QFont, QCloseEvent, QMouseEvent
...
class NotificationWindow(QWidget):
    ...
    def __init__(self, message: str = "", vertical_offset: int = 0, max_scrolls: Optional[int] = None) -> None:
        """初始化通知窗口
        ...
        try:
            ...
            self.max_scrolls = max_scrolls if max_scrolls is not None else int(self.config.get("scroll_count", 3) or 3)
            self.animation: Optional[QPropertyAnimation] = None
            self.text_width = 0
            self.speed = float(self.config.get("scroll_speed", 200.0) or 200.0)  # 从配置中获取滚动速度 (px/s)
            self.space = int(self.config.get("right_spacing", 150) or 150)  # 从配置中获取右侧间隔距离
            self.font_size = float(self.config.get("font_size", 48.0) or 48.0)   # 从配置中获取字体大小
            self.left_margin = int(self.config.get("left_margin", 93) or 93)   # 从配置中获取左侧边距
            self.right_margin = int(self.config.get("right_margin", 93) or 93) # 从配置中获取右侧边距
            self.icon_scale = float(self.config.get("icon_scale", 1.0) or 1.0)      # 从配置中获取图标缩放倍数
            self.label_offset_x = int(self.config.get("label_offset_x", 0) or 0)  # 从配置中获取标签文本x轴偏移
            self.window_height = int(self.config.get("window_height", 128) or 128)  # 从配置中获取窗口高度
            self.label_mask_width = int(self.config.get("label_mask_width", 305) or 305)  # 从配置中获取标签遮罩宽度
            self.vertical_animation: Optional[QPropertyAnimation] = None  # 垂直位置动画
            self.fade_in: Optional[QPropertyAnimation] = None  # 淡入动画
            self.fade_out: Optional[QPropertyAnimation] = None  # 淡出动画

            # 初始化点击交互参数
            self.click_count = 0
            self.click_to_close = int(self.config.get("click_to_close", 3) or 3)  # 从配置中获取触发关闭的点击次数
    ...
    def _create_icon_label(self) -> QLabel:
        ...
        try:
            ...
            icon_label = QLabel()
            icon_size = int(48 * (self.icon_scale or 1.0))  # 根据配置的缩放倍数调整图标大小
            ...
            try:
                ...
                if not icon.isNull():
                    ...
                    # 如果图标加载失败，使用文本图标
                    icon_label.setText("🔊")
                    icon_label.setFont(QFont("Arial", int(32 * (self.icon_scale or 1.0))))
            except Exception as e:
    ...
    def _create_label_widget(self) -> QWidget:
        ...
        label_widget.setFixedWidth(int(self.label_mask_width or 305))
    ...
    def _create_message_text(self, screen_width: int) -> QLabel:
        ...
        try:
            ...
            message_text.setMinimumHeight(int(self.window_height or 128))
            message_text.setMaximumHeight(int(self.window_height or 128))
    ...
    def init_ui(self) -> None:
        ...
        try:
            ...
            banner_opacity = float(self.config.get("banner_opacity", 0.9) or 0.9)
            
            # 创建主容器 - 使用样式表实现半透明背景
            logger.debug("创建主容器")
            self.main_content = QWidget(self)
            self.main_content.setGeometry(0, 0, screen_width, int(self.window_height or 128))
            ...
            layout.setContentsMargins(int(self.left_margin or 93), 0, int(self.right_margin or 93), 0)
            layout.setSpacing(0)
            ...
            self.message_container.setFixedHeight(int(self.window_height or 128))
            
            # 创建消息滑动区域
            logger.debug("创建消息滑动区域")
            slider_width = screen_width - int(self.left_margin or 93) - int(self.label_mask_width or 305) - int(self.right_margin or 93)
            self.message_slider_box = QWidget(self.message_container)
            self.message_slider_box.setGeometry(0, 0, slider_width, int(self.window_height or 128))
    ...
    def setup_animation(self) -> None:
        ...
        try:
            ...
            available_width = screen_width - int(self.left_margin or 93) - int(self.label_mask_width or 305) - int(self.right_margin or 93)
            ...
            if should_scroll:
                ...
                # 计算滚动距离和持续时间
                scroll_distance = self.text_width + available_width + int(self.space or 150)
                scroll_duration = (scroll_distance / float(self.speed or 200.0)) * 1000  # 转换为毫秒
    ...
    def _setup_animation_after_render(self) -> None:
        """在文本完成渲染后设置动画参数"""
        try:
            ...
            # 计算滚动参数
            available_width = screen_width - int(self.left_margin or 93) - int(self.label_mask_width or 305) - int(self.right_margin or 93)
            scroll_distance = screen_width + self.text_width + int(self.space or 150)
            scroll_duration = (scroll_distance / float(self.speed or 200.0)) * 1000  # 转换为毫秒
            ...
            if hasattr(self, 'message_text'):
                ...
                self.animation.setEndValue(QPoint(-(self.text_width + int(self.space or 150)), 0))
                self.animation.setEasingCurve(QEasingCurve.Type.Linear)
                self.animation.finished.connect(self.animation_completed)
                self.animation.start()
                logger.debug(f"滚动动画已启动，持续时间：{int(scroll_duration)} 毫秒")
    ...
    def close_with_animation(self) -> None:
        ...
        try:
            ...
            fade_duration = int(self.config.get("fade_animation_duration", 1500) or 1500)
    ...
    def _on_animation_finished(self) -> None:
        ...
        if self.scroll_count < (self.max_scrolls or 3):
            ...
            move_x = screen_width - int(self.left_margin or 93) - int(self.label_mask_width or 305) - int(self.right_margin or 93)
            self.message_text.move(move_x, 0)  # 使用配置的边距
            if self.animation:
                self.animation.start()
        else:
            # 达到最大滚动次数，延时关闭窗口
    ...
    def closeEvent(self, event: QCloseEvent) -> None:
        ...
        try:
            ...
            event.accept()  # type: ignore 确保窗口能正常关闭
    ...
    def _set_initial_position(self) -> None:
        ...
        try:
            ...
            # 设置窗口位置和尺寸
            self.setFixedHeight(int(self.window_height or 128))
            self.setFixedWidth(screen_width)
            self.move(0, int(self.vertical_offset or 0))
    ...
    def _start_fade_in_animation(self) -> None:
        """启动淡入动画"""
        try:
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        ...
        # 检查是否达到关闭所需的点击次数
        if self.click_count >= (self.click_to_close or 3):
            logger.debug("达到关闭所需点击次数，开始关闭通知窗口")
            self.close_with_animation()
        else:
            remaining_clicks = (self.click_to_close or 3) - self.click_count
            logger.debug(f"尚未达到关闭所需点击次数，还需点击 {remaining_clicks} 次")
